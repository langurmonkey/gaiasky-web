...
If you follow my [BlueSky](https://bsky.app/profile/gaiasky.bsky.social) or [Mastodon](https://mastodon.social/@jumpinglangur) feeds, you have probably already seen most of these features in action. However, for those of you who don't, this post aggregates and summarizes all the information on the upcoming version.
...
First, we have introduced new shading types to billboards in particle groups. Up until now, we shaded all billboards as emissive (additive-blended) blobs or textures. Now, we have a new property, `shadingType`, which can be:
...
We have updated the [FPR asteroids and SSO](/resources/datasets/#catalog-asteroids-fpr) dataset to use the new spherical shading, with rocky textures. The result is a dataset with a much higher graphical fidelity to represent asteroids. Take a look at this video:
...
The result is a dataset of rock and dust particles that rotate around the planet in a realistic manner. It displays the most important features of the structure, like the A ring, the Cassini division, and the B/C/D rings. The final dataset is showcased in the video below.
...
The rings of Saturn scatter light in a diffuse manner. This means that they do not behave like a solid object, but reflect light in pretty much all directions. When the rings are directly lit by the Sun they look as if they *emit* light. We have recreated this phenomenon for particle groups, adding a new optional diffuse scattering coefficient. 3D model materials already had this, but now we need it for the ring dataset.
...
In order to support CJK languages, we have migrated away from hardcoded fonts (created with [Hiero](https://libgdx.com/wiki/tools/hiero), where we generated them offline and added the static `.fnt` and `.png` files to the project) in favor of using the [`gdx-freetype` library](https://libgdx.com/wiki/extensions/gdx-freetype) to autogenerate them. We use the [Noto Sans CJK](https://github.com/notofonts/noto-cjk) font. This ends up being faster to load (we only load the characters that we use) and looking better, while cutting maintenance time.
